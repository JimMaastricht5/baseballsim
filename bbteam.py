...
Team/roster management: lineup construction, pitching rotation, fatigue tracking,
...
class Team:
    def __init__(
        ...
        self.gameplay_pitchers_df = None
    ...
    def batter_index_in_lineup(self, lineup_pos: int = 1) -> int:
        ...
        """
        logger.debug(
            "Getting batter index in lineup. Position: {}, Current lineup: {}", lineup_pos, self.cur_lineup_index_list
        )
    ...
    def calculate_active_defense(self, lineup_df: pd.DataFrame) -> float:
        """
        Sums the Def_WAR of only the active fielders.
        Excludes the Pitcher (handled separately) and the DH (no glove).
        """
    ...
    def dynamic_lineup(self) -> Dict[int64, str]:
        ...
        for position in position_list:
            ...
            pos_index = self.search_for_pos(position=position, lineup_index_list=pos_index_list, stat_criteria=criteria)

            pos_index_list.append(pos_index)
            pos_index_dict[pos_index] = position
    ...
    def set_initial_starting_rotation(self, force_starting_pitcher: None = None) -> None:
        ...
        try:
            if self.starting_pitchers_df is None:  # init starting pitcher list
                self.starting_pitchers_df = self.gameplay_pitchers_df.sort_values(["GS", "IP"], ascending=False).head(5)
    ...
    def set_pitching_condition_post_game(self) -> None:
        """Apply post-game fatigue penalty to pitchers based on outs recorded."""
        self.box_score.set_box_pitching_condition()
        return
    ...
    def batter_stats_in_lineup(self, lineup_order_num: int = 0) -> Series:
        ...
        """
        batting_series = self.gameplay_lineup_df.loc[lineup_order_num]
    ...
    def pitching_change(self, inning: int, score_diff: int) -> int64:
        ...
        if (
            ...
            already_in_game = set(self.box_score.box_pitching.index) | self.used_pitcher_indices
            ...
            if len(available) >= 1:
                ...
                self.gameplay_pitching_df = pitcher_row
                self.box_score.add_pitcher_to_box(pitcher_row)
                self.used_pitcher_indices.add(self.cur_pitcher_index)
                logger.info(
                    f"Emergency pitching change: {pitcher_row['Player'].iloc[0]} "
    ...
    def set_closers(self) -> None:
        ...
        self.relievers_df = (
            self.gameplay_pitchers_df[df_criteria].sort_values(["SV", "ERA"], ascending=[False, True]).head(2)
        )
        return
    ...
    def search_for_pos(
        ...
        not_exhausted = (
            True if ignore_exhaustion else ~(self.gameplay_pos_players_df["Condition"] <= self.fatigue_unavailable)
        )
        ...
        if eligible_subset.empty:
            ...
            last_resort = self.gameplay_pos_players_df[not_in_lineup & not_injured].sort_values(
    ...
    def best_at_stat(
        ...
        stat_index = (
            self.gameplay_pos_players_df[df_criteria].sort_values(stat_criteria, ascending=False).head(count).index
        )
    ...
    def print_starting_lineups(self, current_season_stats: bool = True, show_pitching_starter: bool = True) -> None:
        ...
        if current_season_stats:
            ...
            dfb = bbstats.remove_non_print_cols(self.gameplay_lineup_df)
    ...
    def change_lineup(self, pos_player_bench_hashcode: int, target_batting_order_pos: int) -> None:
        ...
        if pos_player_bench_hashcode in self.gameplay_pos_players_df.index:
            self.insert_player_in_lineup(
                player_hashcode=pos_player_bench_hashcode, target_batting_order_pos=target_batting_order_pos
            )  # insert new player
    ...
    def line_up_dict(self) -> dict:
        ...
        """
        return dict(self.gameplay_lineup_df.iloc[:, 2])  # get pos col w/o name
