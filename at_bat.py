...
class OutCome:
    def __init__(self, debug_b=False) -> None:
        ...
        self.outs_on_play = 0
        self.on_base_b = (
            False  # if this a BB or some form of a hit does not cover GB FC
        )
        self.score_book_cd = ""
        ...
        self.bases_dict = {
            ...
            "LD": 0,
            "SF": 0,
        }  # some outs allow runners to move such as dp or gb
        self.on_base_dict = {
            "BB": True,
            "HBP": True,
            "H": True,
    ...
    def set_score_book_cd(self, cd: str) -> None:
        ...
        self.bases_to_advance = self.bases_dict[
            cd
        ]  # automatically advance x number of bases for all runners
...
class SimAB:
    def __init__(
        self, baseball_data: bbstats.BaseballStats, obp_adjustment=None, debug_b=False
    ) -> None:
        """
        ...
        self.league_GB = 0.429  # ground ball rate for season
        self.league_GB_FC = 0.10  # GB FC occur 10 out of 100 times ball in play
        self.league_FB = 0.372  # fly ball rate for season
        self.league_LD = 0.199  # line drive rate for the season
        ...
        self.HR_adjustment = (
            0.0  # adjust for higher HR rate with new 2023 pitching rules
        )
        self.DBL_adjustment = (
            0.0  # adjust for higher 2B rate with new 2023 pitching rules
        )
        self.dp_chance = 0.20  # 20% chance dp with runner on per mlb
        self.tag_up_chance = 0.20  # 20% chance of tagging up and scoring, per mlb
    ...
    def bb(self) -> bool:
        ...
        batter_bb_rate = (
            ((self.batting.BB + self.BB_adjustment) / self.batting.Total_OB)
            if self.batting.Total_OB > 0
            else (
    ...
    def hbp(self) -> bool:
        ...
        # Safeguard against division by zero - use league average if no data
        batter_hbp_rate = (
            (self.batting["HBP"] / self.batting.Total_Outs)
            if self.batting.Total_Outs > 0
            else float64(self.HBP_rate + self.HBP_adjustment)
        )
        pitcher_hbp_rate = (
            (self.pitching["HBP"] / self.pitching.Total_Outs)
            if self.pitching.Total_Outs > 0
            else float64(self.HBP_rate + self.HBP_adjustment)
        )
    ...
    def hr(self) -> bool:
        ...
        fatigue_hr_boost = (
            self.pitching.Game_Fatigue_Factor * 0.0
        )  # adjust up from zero to incorporate feature

        batter_hr_rate = (
            ((self.batting.HR + self.HR_adjustment) / self.batting.Total_OB)
            if self.batting.Total_OB > 0
            else league_hr_rate
        )
        pitcher_hr_rate = (
            (
                (self.pitching.HR + self.HR_adjustment + fatigue_hr_boost)
                / self.pitching.Total_OB
            )
            if self.pitching.Total_OB > 0
            else league_hr_rate
        )
        ...
        return self.rng() < self.odds_ratio(
            batter_hr_rate, pitcher_hr_rate, league_hr_rate, stat_type="HR"
        )
    ...
    def triple(self) -> bool:
        ...
        batter_3b_rate = (
            (self.batting["3B"] / self.batting.Total_OB)
            if self.batting.Total_OB > 0
            else league_3b_rate
        )
        ...
        return self.rng() < self.odds_ratio(
            hitter_stat=batter_3b_rate,
            pitcher_stat=self.league_3b_pitcher_rate,
            league_stat=league_3b_rate,
            stat_type="3B",
        )
    ...
    def double(self) -> Union[bool, bool_]:
        ...
        # Safeguard against division by zero - use league average if no data
        league_2b_rate = (
            self.league_batting_Total_2B + self.DBL_adjustment
        ) / self.league_batting_Total_OB
        batter_2b_rate = (
            ((self.batting["2B"] + self.DBL_adjustment) / self.batting.Total_OB)
            if self.batting.Total_OB > 0
            else league_2b_rate
        )
        ...
        # Use calculated league rate instead of hardcoded value
        return self.rng() < self.odds_ratio(
            hitter_stat=batter_2b_rate,
            pitcher_stat=self.league_2b_pitcher_rate,
            league_stat=league_2b_rate,
            stat_type="2B",
        )
    ...
    def k(self) -> bool:
        ...
        batter_pa = self.batting.get(
            "PA",
            self.batting.get("AB", 0)
            + self.batting.get("BB", 0)
            + self.batting.get("HBP", 0),
        )
        pitcher_pa = self.pitching.get(
            "PA",
            self.pitching.get("AB", 0)
            + self.pitching.get("BB", 0)
    ...
    def gb_fo_lo(
        ...
        if self.dice_roll <= self.league_GB:
            ...
            gidp_rate = (
                (self.batting["GIDP"] / self.batting["AB"])
                if self.batting["AB"] > 0
                else self.dp_chance
            )
            ...
            if (
                runner_on_first
                and outs <= 1
                and self.rng() <= (gidp_rate + defense_dp_boost)
            ):
                score_book_cd = "DP"
    ...
    def ab_outcome(
        ...
        pa_batter = max(
            batting.Total_OB + (batting.AB - batting.H) + batting.get("SF", 0), 1
        )
    ...
    def odds_ratio(
        ...
        if self.debug_b:
            logger.debug(
                "Computing odds ratio - hitter stat: {}, pitcher stat: {}",
                hitter_stat,
                pitcher_stat,
            )
        ...
        try:
            odds = (
                (hitter_stat / (1 - hitter_stat)) * (pitcher_stat / (1 - pitcher_stat))
            ) / (league_stat / (1 - league_stat))
        except (ZeroDivisionError, RuntimeWarning) as e:
            if isinstance(e, ZeroDivisionError):
                logger.error(
                    "Exception in odds ratio calculation - hitter: {}, pitcher: {}, league: {}, stat_type: {}",
                    hitter_stat,
                    pitcher_stat,
                    league_stat,
                    stat_type,
                ...
                logger.warning(
                    "Warning in odds ratio calculation - hitter: {}, pitcher: {}, league: {}, stat_type: {}",
                    hitter_stat,
                    pitcher_stat,
                    league_stat,
                    stat_type,
...
class MockBaseballStats:
    ...
    def __init__(self):
        ...
        self.league_pa_batting = (
            b_sum["AB"] + b_sum["BB"] + b_sum["HBP"] + b_sum["SF"] + b_sum["SH"]
        )
        ...
        self.league_k_rate_per_ab = b_sum["SO"] / self.league_pa_batting
...
if __name__ == "__main__":
    ...
    print(
        f"Expected Probability Range  : {test_pitcher.OBP:.3f} - {test_batter.OBP:.3f}"
    )
    print("-" * 55)
    ...
    test_pitcher = qualified_pitchers.sort_values("OBP", ascending=False).iloc[
        median_p_idx
    ]
    ...
    def run_def_test(def_war_value):
        ...
        for _ in range(5000):
            sim_ab.ab_outcome(
                test_pitcher, test_batter, outcome, lineup_def_war=def_war_value
            )
            if outcome.on_base_b:
                ob_count += 1
    ...
    print(
        f"Net Defense Impact (Points):   {abs(elite_obp - bad_obp) * 1000:.1f} OBP points"
    )
