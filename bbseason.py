...
"""
Full-season simulation engine. Manages scheduling, multi-threaded game execution,
standings, AI GM assessments, and playoff/World Series brackets.

Classes:
    BaseballSeason - Single league season simulation.
    MultiBaseballSeason - Multiple concurrent leagues with player movement.
"""
...
class OutputCategory:
    """Constants for categorizing simulation output for routing to UI/log elements."""
...
def console_output_handler(category: str, text: str, metadata: Optional[Dict] = None) -> None:
    """Default output handler - prints all output to stdout."""
    print(text, end="")


def null_output_handler(category: str, text: str, metadata: Optional[Dict] = None) -> None:
    """Null handler that discards all output. Useful for silent simulations."""
    pass
...
class BaseballSeason:
    def __init__(
        ...
        self.season_day_num = 0  # placeholder, will be set after partial standings
        ...
        self.teams = (
            list(self.baseball_data.batting_data.Team.unique()) if team_list == [] or team_list is None else team_list
        )
        if len(self.teams) % 2 == 1:  # odd number of teams
            self.teams.append("OFF DAY")

        # Initialize schedule manager
        self.schedule_manager = bbschedule_mgr.ScheduleManager(self.baseball_data, self.new_season)
        ...
        partial_standings = self.baseball_data.populate_standings_from_partial_season()
        for team in self.teams:
            wins_losses = partial_standings.get(team, [0, 0])
        ...
        self.season_day_num = self.schedule_manager.find_start_day(self.team_win_loss, self.team_to_follow)
    ...
    def print_day_schedule(self, day: int) -> str:
        """Print day's schedule in compact 5-column format."""
        schedule_str = ""
        game_day_off = ""
    ...
    def print_standings(self) -> None:
        """Print current standings (W-L, Pct, GB) in compact 3-column format."""
        teaml, winl, lossl = [], [], []
    ...
    def _process_and_print_game_results(self, game_results: List[tuple]) -> None:
        ...
        for game_recap in followed_games:
            self.output_handler(OutputCategory.GAME_RESULT_FULL, game_recap, metadata={"game_recap": game_recap})
    ...
    def update_win_loss(self, away_team_name: str, home_team_name: str, win_loss: List[List[int]]) -> None:
        """Update team standings with game result (thread-safe)."""
    ...
    def get_league_winners(self) -> tuple:
        ...
        def sort_by_record(teams):
            return sorted(teams, key=lambda t: (self.team_win_loss[t][0], -self.team_win_loss[t][1]), reverse=True)
    ...
    def sim_start(self) -> None:
        """Print season start info (teams, schedule length)."""
        teams_paragraph = ""
        self.output_handler(
        ...
        self.output_handler(
            OutputCategory.SEASON_START, f"{teams_paragraph} \n\n", metadata={"team_names": self.team_city_dict}
        )
        if self.season_chatty:
            self.output_handler(
                "season_start", f"Full schedule of games: {self.schedule}\n", metadata={"schedule": self.schedule}
            )
        return

    def sim_end(self) -> None:
        """Print end-of-season info, calculate final stats, and run GM evaluations."""
        ...
        self.output_handler(
            OutputCategory.SEASON_END, f"{self.new_season} Season Standings:\n", metadata={"season": self.new_season}
        )
        self.print_standings()
        self.output_handler(
            OutputCategory.SEASON_END, f"\n{self.new_season} Season Stats\n", metadata={"season": self.new_season}
        )
    ...
    def _perform_gm_evaluations(self) -> None:
        ...
        for team in self.teams:
            ...
            if team and team != "OFF DAY" and team in self.team_win_loss:
                ...
                standings.append({"team": team, "wins": wins, "losses": losses, "win_pct": win_pct})
    ...
    def sim_day_threaded(self, season_day_num: int) -> None:
        """Simulate one day's games across the league using threads."""
        threads = []
        queues = []
        match_ups = []
        ...
        for game in todays_games:  # run all games for a day, day starts at zero
            ...
            if game.completed:
                self.output_handler(
                    OutputCategory.SIM_PROGRESS, f"Skipping {game.away} @ {game.home} (already played)\n", metadata=None
                )
                continue
    ...
    def sim_next_day(self) -> None:
        """Simulate the next day and print standings afterward."""
    ...
    def run_world_series_new(self, al_champ: str, nl_champ: str):
        ...
        self.output_handler(OutputCategory.WORLD_SERIES_END, ws_champion_msg, metadata={"champion": winner})
...
class MultiBaseballSeason:
    def __init__(
        ...
        self.season_chatty = season_chatty
        self.team_to_follow = (
            [season_team_to_follow] if isinstance(season_team_to_follow, str) else (season_team_to_follow or [])
        )
        ...
        if self.minors is not None:
            ...
            self.output_handler(
                OutputCategory.SEASON_START, f"{self.affliations}\n", metadata={"affiliations": self.affliations}
            )
        else:
