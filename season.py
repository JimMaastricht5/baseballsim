...
class BaseballSeason:
    def __init__(self, season_list, team_list, season_length=10):
    ...
    def create_schedule(self):
        # self.schedule = []
        # all_combinations = list(combinations(self.teams, 2))
        # for game_day in range(0, self.season_length - 1):
        # for match_up in all_combinations:
        #     self.schedule.append()
        self.schedule.append([['MIL', 'COL'], ['PIT', 'CIN'], ['CHC', 'STL']])
        self.schedule.append([['MIL', 'CIN'], ['CHC', 'COL'], ['PIT', 'STL']])
        return
    ...
    def sim_season(self):
        # print(self.schedule)
        for game_num in range(0, len(self.schedule)-1):
            game_day = self.schedule[game_num]
            print(game_day)
            for match_up in game_day:
                print(match_up)
                game = bbgame.Game(away_team_name=match_up[0], home_team_name=match_up[1],
                                   baseball_data=self.baseball_data)
                score, inning, win_loss_list = game.sim_game(chatty=True)
            ...
            #     self.team0_season_df = self.team0_season_df[col_list].add(game.teams[0].
...
if __name__ == '__main__':
    ...
    bbseason23 = BaseballSeason(season_list=seasons, team_list=teams, season_length=(2 * len(teams)))  # mult of teams
